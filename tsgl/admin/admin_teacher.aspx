...
<div class="ny-zjhz">
     ...
     <asp:TemplateField HeaderText="<td>课程名</td><td>授课教师</td><td>课程图片</td><td>课程简介</td><td>编辑</td><td>删除</td>" >
    
      <ItemTemplate>
                 
                  <td><%# Eval("title") %> </td><td><%# Eval("uname") %> </td><td><img src="file/<%# Eval("pic") %>" width=100px; height=80px;/></td> <td><%# Eval("nr") %></td><td><asp:HyperLink ID="HyperLink1" Width="60px" Text='编辑' NavigateUrl='<%# "modify_product.aspx?id="+Eval("id") %>'
                                            runat="server"  /> </td>
                      </ItemTemplate>
        ...
        职称：<asp:TextBox ID="price" runat="server"></asp:TextBox><br/> <br/>
