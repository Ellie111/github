...
namespace 服装库存管理系统
{
    public partial class 删除货号信息 : System.Web.UI.Page
    {
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "select")
            {
                ...
                conn.Open();
                string sql = "update [货号表] set 删除标志='" + state + "' where ID =" + id + "";
                OleDbCommand com = new OleDbCommand(sql, conn);
                com.ExecuteNonQuery();
                conn.Close();
                Response.Write("<script>alert('删除货号成功！');window.location='删除货号信息.aspx';</script>");
            }
        }
    }
}